...
def isAdjacent(string):
	...
	char_freq=dict((char,str_list.count(char)) for char in set(str_list)) # Get frequency of char with python dict type
...
def isAdjacent(string):
	...
	if (len(string)+1)//2 < max([str_list.count(char) for char in set(str_list)]):return None
	for perms in permutations(str_list):
		for index, ele in enumerate(perms):
			if index == len(perms)-1: return ''.join(perms)
			elif ele == perms[index + 1]: break
